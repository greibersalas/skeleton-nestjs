 ...
 */
export class Pdf_report_model_state{
    ...
    print(data: any, filtros: any){
        ...
        let page: number = 1;
        pdf.AddPage('P','A4');
        pdf.SetTitle('Reporte de modelos Según estado, HC y Nombre');

        //Titulo
        pdf.SetFont('Arial','B',16);
        pdf.SetY(y);
        pdf.SetX(10);
        pdf.Cell(190,5,`Reporte de modelos Según estado, HC y Nombre`,0,0,'C');
        ...
        data.map((it: any) => {
            ...
            if(cont <= 40){
                ...
                pdf.Cell(25,10,`${moment(elaboration).format('DD/MM/YYYY')}`,1,0,'C',bg);
        ...
        const nameFile: string = `report-model-state-${moment().tz('America/Lima').format('YYYYMMDDHHmmss')}.pdf`;
